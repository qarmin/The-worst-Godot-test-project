...
#
#		#q_OS.alert("afsfqwaasf","asfwfagwas")
#
...
#
#		q_OS.dump_memory_to_file("user://rrr.txt")
#		q_OS.dump_resources_to_file("user://rrr.txt")
#
#		###qq += str(q_OS.execute("godoaft",["asfasf","qwfa"],bool(randi()%2),["ras"],bool(randi()%2)))
#		qq += str(q_OS.find_scancode_from_string("godoafat"))
#
...
#		qq += str(q_OS.get_environment("ENV"))
...
#		qq += str(q_OS.get_unix_time_from_datetime({"year":1992,"month":11,"day":11}))
...
#
#		qq += str(q_OS.has_environment("asfa"))
#		qq += str(q_OS.has_feature("asfqwf"))
...
#		qq += str(q_OS.native_video_play("asf",randf() * 50,"asfqwf","asfqwfqw"))
#		q_OS.native_video_stop()
#		q_OS.native_video_unpause()
#
#		q_OS.open_midi_inputs()
#
#		q_OS.print_all_resources("asfasfqwrq")
#		q_OS.print_all_textures_by_size()
#		q_OS.print_resources_by_type(PoolStringArray(["asfqw","afqw"]))
