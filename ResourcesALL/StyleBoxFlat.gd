	...
	if counter <= 0:
		...
		if !Autoload.RANDI:
			...
			if Autoload.WRONG_BUGS:
				...
				q_StyleBoxFlat.set_draw_center(bool(randi()%2))
				for i in range(4):
					q_StyleBoxFlat.set_border_width(i,randi() % Autoload.RANGE - Autoload.RANGE / 2)
				q_StyleBoxFlat.set_border_color(Color(randf(),randf(),randf(),randf()))
				q_StyleBoxFlat.set_border_blend(bool(randi()%2))
				for i in range(4):
					q_StyleBoxFlat.set_corner_radius(i,randi() % Autoload.RANGE - Autoload.RANGE / 2)
				q_StyleBoxFlat.set_corner_detail(randi() % Autoload.RANGE - Autoload.RANGE / 2)
				for i in range(4):
					q_StyleBoxFlat.set_expand_margin(i,randi() % Autoload.RANGE - Autoload.RANGE / 2)
				q_StyleBoxFlat.set_shadow_color(Color(randf(),randf(),randf(),randf()))
