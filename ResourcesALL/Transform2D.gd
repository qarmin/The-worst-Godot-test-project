...
#				q_Transform2D = Transform2D(Transform( Autoload.get_vector3(),Autoload.get_vector3(),Autoload.get_vector3(),Autoload.get_vector3()))
		...
		if randi() % 2 == 1:
			q_Transform2D.affine_inverse())

		if randi() % 2 == 1:
			q_Transform2D.basis_xform( Autoload.get_vector2()))
		if randi() % 2 == 1:
			q_Transform2D.basis_xform_inv( Autoload.get_vector2()))

		if randi() % 2 == 1:
			q_Transform2D.get_origin())
		if randi() % 2 == 1:
			q_Transform2D.get_rotation())
		if randi() % 2 == 1:
			q_Transform2D.get_scale())

		if randi() % 2 == 1:
			q_Transform2D.interpolate_with( Autoload.get_transform2d(), Autoload.get_float()))
		if randi() % 2 == 1:
