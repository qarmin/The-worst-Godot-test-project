...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if randi() % 2 == 1:
			qq += str(q_UPNPDevice.add_port_mapping( Autoload.get_int(), Autoload.get_int(), Autoload.get_string(), Autoload.get_string(), Autoload.get_int() ))
		if randi() % 2 == 1:
			qq += str(q_UPNPDevice.delete_port_mapping( Autoload.get_int(), Autoload.get_string() ))
		if randi() % 2 == 1:
