...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if Autoload.WRONG_BUGS:
			...
			qq += str(q_OpenSimplexNoise.get_noise_1d( randf() * 50 - 25 ))
			qq += str(q_OpenSimplexNoise.get_noise_2d( randf() * 50 - 25,randf() * 50 - 25 ))
			qq += str(q_OpenSimplexNoise.get_noise_2dv( Vector2(randf() * 50 - 25,randf() * 50 - 25)))
