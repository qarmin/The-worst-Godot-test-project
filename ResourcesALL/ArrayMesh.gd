...
func alt_process(delta) -> void:
	...
	if counter <= 0:
		...
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.get_blend_shape_count())
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.get_blend_shape_name( Autoload.get_int()))
#SPAM		if randi() % 2 == 1:
#			Autoload.qq = str(q_ArrayMesh.lightmap_unwrap( Autoload.get_transform(),Autoload.get_float()))
		if randi() % 2 == 1:
			q_ArrayMesh.regen_normalmaps()
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.surface_find_by_name( Autoload.get_string() ))
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.surface_get_array_index_len( Autoload.get_int() ))
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.surface_get_array_len( Autoload.get_int() ))
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.surface_get_format( Autoload.get_int() ))
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.surface_get_name( Autoload.get_int() ))
		if randi() % 2 == 1:
			Autoload.qq = str(q_ArrayMesh.surface_get_primitive_type( Autoload.get_int() ))
		if randi() % 2 == 1:
