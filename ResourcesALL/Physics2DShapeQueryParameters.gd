...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if randi() % 2 == 1:
			q_Physics2DShapeQueryParameters.set_transform(Transform2D(Vector2(Autoload.get_float(),randf() * 50),Autoload.get_vector2(),Vector2(Autoload.get_float(),randf() * 50)))
		if randi() % 2 == 1:
