...
func alt_process(delta) -> void:
	...
	if counter <= 0:
		...
		nodeFunction(q_Image,true)

func nodeFunction(q_Image : Image, can_reset : bool = false) -> void:
	
	if can_reset:
		if randi() % 2 == 1:
			q_Image = Image.new()
	if randi() % 2 == 1:
		AutoObjects.A_Object(q_Image)
		AutoResourcesResource.nodeFunction(q_Image)
		#BUG NEW
#	### START TEMP
#
...
#
#	if randi() % 2 == 1:
#		q_Image.blend_rect( temp_Image, Autoload.get_rect2f(1000.0), Autoload.get_vector2f(1000.0))
#	if randi() % 2 == 1:
#		q_Image.blend_rect_mask( temp_Image, temp_Image, Autoload.get_rect2f(1000.0), Autoload.get_vector2f(1000.0))
#
##	BUG	if randi() % 2 == 1:
##			q_Image.blit_rect( temp_Image, Autoload.get_rect2f(1000.0),Autoload.get_vector2f(1000.0))
#	if randi() % 2 == 1:
#		q_Image.blit_rect_mask( temp_Image, temp_Image, Autoload.get_rect2f(1000.0),Autoload.get_vector2f(1000.0))
#
##	BUG	if randi() % 2 == 1:
