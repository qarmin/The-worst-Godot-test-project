	...
	if counter <= 0:
		...
		if !Autoload.RANDI:
			...
			if Autoload.WRONG_BUGS:
				...
				for i in range(4):
					q_StyleBoxTexture.set_margin_size(i,randf() * Autoload.RANGE - Autoload.RANGE / 2)
					q_StyleBoxTexture.set_expand_margin_size(i,randf() * Autoload.RANGE - Autoload.RANGE / 2)
