...
func nodeFunction(q_PackedFloat32Array: PackedFloat32Array, can_reset: bool = false) -> void:
	...
	if randi() % 2 == 1:
		q_PackedFloat32Array.sort()
	if randi() % 2 == 1:
		q_PackedFloat32Array.set(Autoload.get_int(), Autoload.get_int())
	if randi() % 2 == 1:
		q_PackedFloat32Array.to_byte_array()
