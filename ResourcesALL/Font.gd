...
#			q_Font.draw( RID(), Vector2(Autoload.get_randf(),Autoload.get_randf()), Autoload.get_string(), Color(randf(),randf(),randf(),randf()), Autoload.get_randi(), Color(randf(),randf(),randf(),randf()))
...
#			qq += str(q_Font.get_wordwrap_string_size( Autoload.get_string(), Autoload.get_randf()))
#
