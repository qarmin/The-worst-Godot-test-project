...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if randi() % 2 == 1:
			qq += str(q_Mesh.create_outline( Autoload.get_float()))
		if randi() % 2 == 1:
		...
		if randi() % 2 == 1:
			qq += str(q_Mesh.surface_get_arrays( Autoload.get_int() ))
		if randi() % 2 == 1:
			qq += str(q_Mesh.surface_get_blend_shape_arrays( Autoload.get_int() ))
		if randi() % 2 == 1:
