...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if Autoload.WRONG_BUGS:
			qq += str(q_Mesh.create_convex_shape())
			qq += str(q_Mesh.create_outline( randf() * 1000 - 500))
			qq += str(q_Mesh.create_trimesh_shape())
			qq += str(q_Mesh.generate_triangle_mesh())
			qq += str(q_Mesh.get_faces())
			qq += str(q_Mesh.get_surface_count())
			qq += str(q_Mesh.surface_get_arrays( randi() % 1000 - 500 ))
			qq += str(q_Mesh.surface_get_blend_shape_arrays( randi() % 1000 - 500 ))
			qq += str(q_Mesh.surface_get_material( randi() % 1000 - 500 ))
			q_Mesh.surface_set_material( randi() % 1000 - 500, SpatialMaterial.new())
