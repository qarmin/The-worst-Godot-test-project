...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if randi() % 2 == 1:
			Autoload.qq = str(get_offset_buttons_visible())
		if randi() % 2 == 1:
			Autoload.qq = str(get_select_with_rmb())
		if randi() % 2 == 1:
		...
		if randi() % 2 == 1:
			Autoload.qq = str(get_tab_offset())
		if randi() % 2 == 1:
			Autoload.qq = str(get_tab_rect(Autoload.get_int()))
		if randi() % 2 == 1:
