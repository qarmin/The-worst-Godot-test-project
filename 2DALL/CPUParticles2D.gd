...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if randi() % 2 == 1:
			set_normalmap(Autoload.loadA("res://RES/Sprite.png"))
		if randi() % 2 == 1:
			set_emission_shape(Autoload.get_randi())
		if randi() % 2 == 1:
			set_emission_sphere_radius(Autoload.get_randf())
		if randi() % 2 == 1:
			set_emission_rect_extents(Vector2(Autoload.get_randf(), Autoload.get_randf()))
		if randi() % 2 == 1:
			set_emission_points(PoolVector2Array([Vector2(Autoload.get_randf(), Autoload.get_randf()),Vector2(Autoload.get_randf(), Autoload.get_randf())]))
		if randi() % 2 == 1:
			set_emission_normals(PoolVector2Array([Vector2(Autoload.get_randf(), Autoload.get_randf()),Vector2(Autoload.get_randf(), Autoload.get_randf())]))
		if randi() % 2 == 1:
			set_emission_colors(PoolColorArray([Color(randf(),randf(),randf(),randf())]))
		if randi() % 2 == 1:
			set_particle_flag(Autoload.get_randi(),Autoload.get_randi())
		if randi() % 2 == 1:
			set_spread(Autoload.get_randf())
		if randi() % 2 == 1:
			set_flatness(Autoload.get_randf())
		if randi() % 2 == 1:
			set_gravity(Vector2(Autoload.get_randf(), Autoload.get_randf()))
		if randi() % 2 == 1:
			set("local_coords", bool(randi()%2))
		if randi() % 2 == 1:
			set("draw_order", Autoload.get_randi())
		if randi() % 2 == 1:
			set("texture", (Autoload.loadA("res://RES/Sprite.png")))
		if randi() % 2 == 1:
			set("normalmap", (Autoload.loadA("res://RES/Sprite.png")))
		if randi() % 2 == 1:
			set("emission_shape", Autoload.get_randi())
		if randi() % 2 == 1:
			set("flag_align_y", bool(randi()%2))
			
		for j in range(12): # range(12):
			if randi() % 2 == 1:
				set_param(j,Autoload.get_randf())
				set_param_curve(j,Autoload.loadA("res://RES/Curve.tres"))
				set_param_randomness(j,Autoload.get_randf())
