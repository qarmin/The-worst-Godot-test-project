...
func nodeFunction(q_KinematicBody2D: KinematicBody2D, can_reset: bool = false) -> void:
	...
	if Autoload.SLOW_FUNCTIONS:
		...
		if randi() % 2 == 1:
			q_KinematicBody2D.move_and_slide_with_snap(
				Autoload.get_vector2(), Autoload.get_vector2(), Autoload.get_vector2(), Autoload.get_bool(), Autoload.get_int(), Autoload.get_float(), Autoload.get_bool()
			)
