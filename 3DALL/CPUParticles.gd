...
func _process(delta) -> void:
	...
	#			set_flag(i,bool(randi()%2))
		...
		if Autoload.WRONG_BUGS:
			...
			set_particle_flag(randi() % 1000 - 500,bool(randi()%2))
				
			set_spread(randf() * 1000 - 500)
			set_flatness(randf() * 1000 - 500)
			set_gravity(Vector3(randf() * 1000 - 500,randf() * 1000 - 500,randf() * 1000 - 500))

			set_param(randi() % 1000 - 500,randf() * 1000 - 500)
			set_param_curve(randi() % 1000 - 500,load("res://RES/Curve" + str(randi()%3+1) + ".tres"))
			set_param_randomness(randi() % 1000 - 500,randf() * 1000 - 500)
			...
			if Autoload.SLOW_FUNCTIONS:
				...
				set_flag(randi() % 1000 - 500,bool(randi()%2))
