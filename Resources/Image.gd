...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if Autoload.WRONG_BUGS:
			...
			q_Image.blend_rect( Image.new(), Rect2(Vector2(randf() * 1000 - 500,randf() * 1000 - 500),Vector2(randf() * 1000 - 500,randf() * 1000 - 500)), Vector2(randf() * 1000 - 500,randf() * 1000 - 500) )
			q_Image.blend_rect_mask( Image.new(), Image.new(), Rect2(Vector2(randf() * 1000 - 500,randf() * 1000 - 500),Vector2(randf() * 1000 - 500,randf() * 1000 - 500)), Vector2(randf() * 1000 - 500,randf() * 1000 - 500) )
			
			q_Image.blit_rect( Image.new(), Rect2(Vector2(randf() * 1000 - 500,randf() * 1000 - 500),Vector2(randf() * 1000 - 500,randf() * 1000 - 500)), Vector2(randf() * 1000 - 500,randf() * 1000 - 500) )
			q_Image.blit_rect_mask( Image.new(), Image.new(), Rect2(Vector2(randf() * 1000 - 500,randf() * 1000 - 500),Vector2(randf() * 1000 - 500,randf() * 1000 - 500)), Vector2(randf() * 1000 - 500,randf() * 1000 - 500) )
			
			q_Image.bumpmap_to_normalmap( randf() * 1000 - 500)
			q_Image.clear_mipmaps()
			qq += str(q_Image.compress( randi() % 1000 - 500, randi() % 1000 - 500, randf() * 1000 - 500)) #compressMode, #CompressSource
			q_Image.convert(randi() % 1000 - 500) #Format
			q_Image.copy_from( Image.new() )
			
			q_Image.create( randi() % 1000 - 500, randi() % 1000 - 500, bool(randi()%2), randi() % q_Image.FORMAT_MAX ) #Format
			q_Image.create_from_data( randi() % 1000 - 500, randi() % 1000 - 500, bool(randi()%2), randi() % 1000 - 500 , PoolByteArray([11,124,1241,24,21,214,12,11]))#Format
			
			q_Image.crop( randi() % 1000 - 500, randi() % 1000 - 500 )
			qq += str(q_Image.decompress())
			qq += str(q_Image.detect_alpha())
			q_Image.expand_x2_hq2x()
			q_Image.fill( Color(randf(),randf(),randf(),randf()))
			...
			qq += str(q_Image.get_format())
			qq += str(q_Image.get_height())
			qq += str(q_Image.get_mipmap_offset( randi() % 1000 - 500 ))
			qq += str(q_Image.get_pixel( randi() % 1000 - 500, randi() % 1000 - 500 ))
			qq += str(q_Image.get_pixelv( Vector2(randf() * 1000 - 500,randf() * 1000 - 500)))
			qq += str(q_Image.get_rect( Rect2(Vector2(randf() * 1000 - 500,randf() * 1000 - 500),Vector2(randf() * 1000 - 500,randf() * 1000 - 500))))
			qq += str(q_Image.get_size())
			qq += str(q_Image.get_used_rect())
			qq += str(q_Image.get_width())
			...
			q_Image.resize( randi() % 1000 - 500, randi() % 1000 - 500, randi() % 5)  #Interpolation
			q_Image.resize_to_po2( bool(randi()%2))
			
			qq += str(q_Image.rgbe_to_srgb())
			qq += str(q_Image.save_png( "res://lafar.png" ))
			
			q_Image.set_pixel( randi() % 1000 - 500, randi() % 1000 - 500,Color(randf(),randf(),randf(),randf()))
			q_Image.set_pixelv( Vector2(randf() * 1000 - 500,randf() * 1000 - 500),Color(randf(),randf(),randf(),randf()))
