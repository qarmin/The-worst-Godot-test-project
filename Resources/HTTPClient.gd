...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if Autoload.WRONG_BUGS:
			...
			q_HTTPClient.close()
			qq += str(q_HTTPClient.connect_to_host( "Szczur", randi() % 1000 - 500, bool(randi()%2), bool(randi()%2) ))
			...
			qq += str(q_HTTPClient.is_response_chunked())
			qq += str(q_HTTPClient.poll())
			qq += str(q_HTTPClient.query_string_from_dict( {"Ouo": randi() % 1000 - 500, "Kompresja": 124}))
			qq += str(q_HTTPClient.read_response_body_chunk())
			
			qq += str(q_HTTPClient.request( randi() % 1000 - 500, "Strona", PoolStringArray(["Tak","Nie","Być może"]), "Coś innego" ))#Method
			qq += str(q_HTTPClient.read_response_body_chunk())
			qq += str(q_HTTPClient.request_raw( randi() % 1000 - 500, "Strona", PoolStringArray(["Tak","Nie","Być może"]), PoolByteArray([11,124,1241,24,21,214,12,11]) ))#Method
			
			q_HTTPClient.set_read_chunk_size( randi() % 1000 - 500 )
