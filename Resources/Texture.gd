...
func _process(delta) -> void:
	...
	if counter <= 0:
		...
		if Autoload.WRONG_BUGS:
			q_Texture.draw(RID(),Vector2(randf() * 1000 - 500, randf() * 1000 - 500),Color(randf(),randf(),randf(),randf()),bool(randi()%2),Texture.new())
			q_Texture.draw_rect(RID(),Rect2(Vector2(randf() * 1000 - 500, randf() * 1000 - 500),Vector2(randf() * 1000 - 500, randf() * 1000 - 500)),bool(randi()%2),Color(randf(),randf(),randf(),randf()),bool(randi()%2),Texture.new())
			q_Texture.draw_rect_region(RID(),Rect2(Vector2(randf() * 1000 - 500, randf() * 1000 - 500),Vector2(randf() * 1000 - 500, randf() * 1000 - 500)),Rect2(Vector2(randf() * 1000 - 500, randf() * 1000 - 500),Vector2(randf() * 1000 - 500, randf() * 1000 - 500)), Color(randf(),randf(),randf(),randf()), bool(randi()%2),Texture.new(),bool(randi()%2))
